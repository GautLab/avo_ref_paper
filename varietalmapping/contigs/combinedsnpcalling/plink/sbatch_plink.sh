...
#Antilion Pop
#vcftools --gzvcf $(basename ${VCF} .clean.vcf.gz).noBAD.recodeall.maf.recode.vcf.gz --keep ${POP3} --out $(basename ${VCF} .clean.vcf.gz).noBAD.ANT --recode --recode-INFO-all
#pigz -p ${CORES} $(basename ${VCF} .clean.vcf.gz).noBAD.ANT.recode.vcf
#vcftools --gzvcf $(basename ${VCF} .clean.vcf.gz).noBAD.ANT.recode.vcf.gz --maf 0.05 --recode --recode-INFO-all --out $(basename ${VCF} .clean.vcf.gz).noBAD.ANT.recodeall.maf
#pigz -p ${CORES} $(basename ${VCF} .clean.vcf.gz).noBAD.ANT.recodeall.maf.recode.vcf
