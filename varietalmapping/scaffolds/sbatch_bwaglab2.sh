#!/bin/bash
#SBATCH -J bwaglab		# jobname
#SBATCH -o bwaglab.o%A.%a		# jobname.o%j for single (non array) jobs jobname.o%A.%a for array jobs
#SBATCH -e bwaglab.e%A.%a		# error file name A is the jobid and a is the arraytaskid
#SBATCH -a 1-24	#24, 13		# start and stop of the array start-end
#SBATCH --nodes=1
#SBATCH -c 16
#SBATCH -p p1,gcluster			# queue (partition) -- compute, shared, large-shared, debug (30mins)
###SBATCH --mem-per-cpu=4000
###SBATCH -t 08:00:00		# run time (dd:hh:mm:ss) - 1.5 hours
...
#PATH=/jet/home/solares/bin/bwa-0.7.17:$PATH
#module load samtools/1.11.0
PATH=/gpool/bin/bwa-0.7.17:/gpool/bin/samtools-1.11/bin:$PATH
...
#QRY1="illuminapnas/$(basename $QRY1 .fastq.gz)"
#QRY2="illuminapnas/$(basename $QRY2 .fastq.gz)"
...
echo "Starting alignment for ${PREFIX} on ${REF} with ${OPTIONS}"
...
pigz -p ${CORES} $(basename ${REF} .fasta)_${PREFIX}A.sam
pigz -p ${CORES} $(basename ${REF} .fasta)_${PREFIX}B.sam

###HASS
#bwa mem -M ${OPTIONS} $REF ${QRY1}${PAIRED} ${QRY2}${PAIRED} > $(basename ${REF} .fasta)_${PREFIX}.sam
#pigz -p ${CORES} $(basename ${REF} .fasta)_${PREFIX}.sam
