...
BAMFILE=$(basename ${BAMFILEA} -2_sorted.bam).merged.bam

LOCAL="./"
TEMPDIR=${LOCAL}_GLAB_${SLURM_ARRAY_TASK_ID}
SORTOPTS="--threads ${CORES} -m 3G -T ${TEMPDIR}"

samtools merge -@ ${CORES} ${BAMFILE} ${BAMFILEA} ${BAMFILEB}
