...
#sort first
#cat $(basename $REF .fasta)_$(basename $CDSFILE .cds).psl | sort -n -k 16,16 | sort -s -k 14,14 > $(basename $REF .fasta)_$(basename $CDSFILE .cds).sorted.psl
#blat2hints.pl --in=$(basename $REF .fasta)_$(basename $CDSFILE .cds).sorted.psl --out=$(basename $REF .fasta)_$(basename $CDSFILE .cds).hints.gff
#augustus --species=$SPECIES --hintsfile=$(basename $REF .fasta)_$(basename $CDSFILE .cds).hints.gff --extrinsicCfgFile=extrinsic.ME.cfg $REF
augustus --species=$SPECIES --hintsfile=$(basename $REF .fasta)_$(basename $CDSFILE .cds).hints.gff --extrinsicCfgFile=extrinsic.MPE.cfg $REF
