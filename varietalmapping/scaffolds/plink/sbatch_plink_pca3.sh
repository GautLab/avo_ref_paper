...
NOOVERLAPSAMPLES=noovlsamples.txt
...
#New groups for PCA analysis for removing possible confounding factors
#no outgroups and no problem varities
vcftools --gzvcf ${VCF} --keep ${NOOVERLAPSAMPLES} --out $(basename ${VCF} .vcf.gz).noOVLOG --recode --recode-INFO-all
pigz -p ${CORES} $(basename ${VCF} .vcf.gz).noOVLOG.recode.vcf
vcftools --gzvcf $(basename ${VCF} .vcf.gz).noOVLOG.recode.vcf.gz --maf 0.05 --recode --recode-INFO-all --out $(basename ${VCF} .vcf.gz).noOVLOG.recodeall.maf
pigz -p ${CORES} $(basename ${VCF} .vcf.gz).noOVLOG.recodeall.maf.recode.vcf
vcftools --gzvcf $(basename ${VCF} .vcf.gz).noOVLOG.recodeall.maf.recode.vcf.gz --plink --out $(basename ${VCF} .vcf.gz).noOVLOG.recodeall.maf.recode
plink --file $(basename ${VCF} .vcf.gz).noOVLOG.recodeall.maf.recode --memory 30000 --threads ${CORES} --pca --out $(basename ${VCF} .vcf.gz).noOVLOG.recodeall.maf.recode.PCA
