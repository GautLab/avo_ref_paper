...
singularity exec instance://BRAKER braker.pl --cores=${CORES} --species=${SPECIES} --genome=${REF} --bam=${BAM} --prot_seq=${PROTEINFILE} --prg=gth --softmasking --AUGUSTUS_BIN_PATH=/usr/local/bin --AUGUSTUS_CONFIG_PATH=${AUGUSTUS_CONFIG_PATH} --AUGUSTUS_SCRIPTS_PATH=/opt/augustus-3.4.0/scripts --workingdir=${MOUNTDIR}
#singularity exec instance://BRAKER braker.pl --species=${SPECIES} --genome=${REF} --softmasking --bam=${BAM} --prg=gth --AUGUSTUS_BIN_PATH=/usr/local/bin --AUGUSTUS_CONFIG_PATH=${AUGUSTUS_CONFIG_PATH} --AUGUSTUS_SCRIPTS_PATH=/opt/augustus-3.4.0/scripts --workingdir=${MOUNTDIR}
