#!/bin/bash
#SBATCH -J hisat2build		# jobname
#SBATCH -o hisat2build.o%A.%a		# jobname.o%j for single (non array) jobs jobname.o%A.%a for array jobs
#SBATCH -e hisat2build.e%A.%a		# error file name A is the jobid and a is the arraytaskid
...
#Build index. Run only on first job
hisat2-build -p ${SLURM_CPUS_PER_TASK} ${REF} ${REF}
